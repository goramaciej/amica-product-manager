            ...
            <div class="filters">
                ...
                <div class="filter-search filter">
                    ...
                    <div class="form-group">
                        ...
                        />
                        <i
                            class="icofont-close-circled icon"
                            v-if="clearSearchButton"
                            @click="clearSearch"
                        ></i>
                        <div class="search-results">
                            <div
                                v-if="searchItemWarning.length > 0"
                                class="search-result disabled"
                            >{{ searchItemWarning }}</div>
                            <div
                                class="search-result"
...
export default {
    ...
    computed: {
        allProducts() {
            return JSON.parse(JSON.stringify(this.$store.getters.products));
        },
        products: {
            get() {
    ...
    methods: {
        ...
        changeCategory() {
            ...
            const obj = this.categories.find(
                el => el.alias === this.$route.params.category
            );
            if (obj) {
        ...
        searchTextChanged(ev) {
            ...
            if (tex.length > 1) {
                this.searchArr = this.allProducts.filter(item =>
                    item.description.toLowerCase().includes(tex.toLowerCase())
                );
